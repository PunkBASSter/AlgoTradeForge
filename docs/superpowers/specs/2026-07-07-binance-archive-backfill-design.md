...
- **Phase 3 — taker-volume source & value semantics changed:** the retired live collector sourced `/futures/data/takerlongshortRatio` (Binance's pre-computed `buySellRatio`/`buyVol`/`sellVol`); the materializer now DERIVES from kline quote columns — `buy_vol_usd = taker_buy_quote_volume [10]`, `sell_vol_usd = Math.Max(0, quote_volume [7] − [10])` (clamped: float subtraction of near-equal quote volumes can go slightly negative, which must read as zero sells, not a negative volume), `ratio = sell_vol_usd > 0 ? buy/sell : 0`. Columns/path/schema are byte-identical, so existing configs keep reading it, but ABSOLUTE VALUES differ from the old endpoint — months collected live pre-Phase-3 (HistoryStart 2026-03…) are silently re-materialized to kline-derived values on the next backfill. Accepted (verify comparability in the Task-14 smoke); no replace-guard helps here (row counts match, only values differ).
