...
`IVenueConnector` returns `IAsyncEnumerable<IMarketEvent>`; each event boxes an `IMarketEvent` carrier and switches on type **at the ingest seam** (once per event, not per frame). For Binance live (tens of instruments, a few `aggTrade`/sec each) this allocation is immaterial. The GC-free upgrade — a push/visitor `IVenueSink.OnTrade(in TradeTick)`/`OnQuote(in QuoteTick)` model that eliminates the boxing — matters only at the ~1000-instrument firehose (IB). **Decision: ship the pull seam now; measure with the Plan-1 firehose benchmark; upgrade to the push model at a recorded trigger** (seam allocation threshold or IB onboarding, whichever first). The relay's per-frame hot path is already zero-boxing (compile-time generics); only the seam carrier is at issue.
