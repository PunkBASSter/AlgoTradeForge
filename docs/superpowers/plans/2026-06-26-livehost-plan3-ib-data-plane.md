...
- Produces:
```csharp
internal sealed class IbSession(IIbMarketDataClient client, IbWrapper wrapper)
{
...
```
On `wrapper.ConnectionDropped`, the session re-`Connect`s, re-issues every tracked subscription under fresh reqIds (re-registering its sink), then raises `Reconnected` so bar sources can trigger catch-up.
...
public class IbSessionTests
{
    ...
    [Fact]
    public void ConnectionDropped_Reconnects_AndResubscribesAll_ThenRaisesReconnected()
    {
        var client = new FakeClient();
        var wrapper = new IbWrapper();
        var session = new IbSession(client, wrapper);
        ...
        int reconnected = 0;
        session.Reconnected += () => reconnected++;

        wrapper.connectionClosed(); // drop
...
// plane (Plan 4) both hold a handle to this. Reconnect is driven by IbWrapper.ConnectionDropped; the
// callbacks fire on the single EReader pump thread, so the subscription map is guarded by a lock.
internal sealed class IbSession
{
    ...
    private void OnConnectionDropped()
    {
        ...
        _client.Connect().GetAwaiter().GetResult(); // pump-thread callback; the connect retry loop is itself async-over-sync-safe here
...
> **Concurrency note for the reviewer:** `OnConnectionDropped` runs on the EReader pump thread (via `connectionClosed`). `_client.Connect()` blocks that thread until the new socket is up — acceptable because the dropped pump has already exited its loop and a *new* pump starts inside `Connect`. The `GetAwaiter().GetResult()` is the one sanctioned sync-over-async point (a pump-thread event handler cannot be async); annotate it with the inline comment shown. Re-issuing under the SAME reqIds is intentional — the new socket has no prior request state, and reusing ids keeps the sink registrations valid.
