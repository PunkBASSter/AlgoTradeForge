...
internal sealed class SymbolCollector(
    ...
    BinanceSpotClient? spotClient,
    CandleCsvWriter candleWriter,
    FeedCsvWriter feedWriter,
    FeedSchemaManager schemaManager,
    ILogger<SymbolCollector> logger)
{
        ...
        CancellationToken ct)
    {
        ...
        if (!isSpot)
        {
            schemaManager.EnsureSchema(assetDir, "candle-ext", new FeedCollectionConfig
            {
                Name = "candle-ext",
                Interval = interval
            }, CandleExtColumns);
        ...
        long expectedMs = string.IsNullOrEmpty(interval)
            ? 0
            : (long)BinanceIntervalMap.ToTimeSpan(interval).TotalMilliseconds;
        ...
        CancellationToken ct)
    {
        ...
        schemaManager.EnsureSchema(assetDir, "funding-rate", new FeedCollectionConfig
        {
            Name = "funding-rate",
            Interval = ""
        }, FundingRateColumns, new AlgoTradeForge.Domain.History.AutoApplyDefinition
        {
            Type = "FundingRate",
            RateColumn = "rate"
        });
        ...
        CancellationToken ct)
    {
        ...
        schemaManager.EnsureSchema(assetDir, "mark-price", new FeedCollectionConfig
        {
            Name = "mark-price",
            Interval = interval
        }, MarkPriceColumns);
        ...
        CancellationToken ct)
    {
        ...
        schemaManager.EnsureSchema(assetDir, feedName, new FeedCollectionConfig
        {
            Name = feedName,
            Interval = interval
        }, columns);
    ...
    private static void UpdateFeedStatus(
        string assetDir,
        string feedName,
        string interval,
        long? firstTs,
    ...
    {
        var existing = FeedStatusManager.Load(assetDir, feedName);
        ...
        FeedStatusManager.Save(assetDir, feedName, status);
    }
}
