...
public sealed class BinanceWebSocketManager : IAsyncDisposable
{
    ...
    public Task SubscribeAggTrade(string symbol, Action<BinanceAggTrade> onMessage)
    {
        var stream = $"{symbol.ToLowerInvariant()}@aggTrade";
        var url = $"{_wsUrl}/ws/{stream}";
        return ConnectStream(url, stream, buffer =>
        {
            var msg = JsonSerializer.Deserialize<BinanceAggTrade>(buffer.Span, BinanceJsonOptions.Default);
            if (msg is not null)
                onMessage(msg);
        });
    }
