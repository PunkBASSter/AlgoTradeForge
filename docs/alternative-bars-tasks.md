...
- [x] **P4-19** Optimization Primary becomes multi-select chip; estimated run count = `primaries × param_combos`. (TRD §10.2) — `frontend/components/features/launch/multi-primary-picker.tsx` chip wrapper. Cost preview label rendered inline as `"N × combos = trials"` next to the Primary section header. Submits a single DSS containing `[...primaries, ...sides]`; server-side `ExpandMultiPrimary` fans the multi-primary case into N child runs (TRD §9.6). `RunNewPanel` rewired: `dssValue: DataSubscription[][]` removed, replaced with `primaries` + `sides` lists with bidirectional CodeMirror JSON sync. Legacy `dss-builder.tsx` deleted.
