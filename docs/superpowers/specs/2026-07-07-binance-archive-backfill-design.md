...
- **Update `docs/service-decomposition-vision.md` §HL@cloud:** its "24/7 collectors for un-backfillable feeds (OI, ratios, taker vol, liquidations, funding)" list predates this audit. Under the new classification only `liquidations` + spot `book-ticker` are un-backfillable; OI/ratios/taker/funding are replenishable from the archive. The HL@cloud eager set and the pull-sync volume shrink accordingly.
- **Cloud profile ↔ `Eager` override linkage:** the archive publishes with ~1 day lag, so replenishable ≠ available-in-real-time. For live warm-up the cloud instance keeps klines (and anything warm-up-critical) eager via the per-feed `"Eager": true` override — this is the intended mechanism, record it in the vision doc when updating.
