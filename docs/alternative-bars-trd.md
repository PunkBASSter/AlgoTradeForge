...
PartitionedSourceReader                 (enumerates candles/<YYYY>-<MM>_<src>.csv
    ...
    │                                    when the requested type needs taker-buy
    │                                    flow (EqI from time bars; EqT/EqV/EqD/EqI
    │                                    that opt into a sidecar) AND the asset
    │                                    has a `candle-ext` directory (today:
    │                                    futures/perps only — see §11),
    │                                    simultaneously enumerates
    │                                    candle-ext/<YYYY>-<MM>_<src>.csv and
    │                                    joins by `ts` 1:1.  Spot time-bar
    │                                    sources have no `candle-ext`, so the
    │                                    join is skipped — EqI is already
    │                                    rejected at eligibility-check time
    │                                    (§11), and EqT/EqV/EqD do not need
    │                                    flow data.  Tick sources never join
    │                                    `candle-ext` at all, since per-tick
    │                                    `is_buyer_maker` is intrinsic.
...
- **From a time-bar source (`1m`, `1h`, …):** `O` = first source record's open; `H` = max of records' highs; `L` = min of records' lows; `C` = last source record's close; volumes summed. The buy/sell split is derived from the **paired `candle-ext` row** joined by `ts` (`taker_buy_vol` column → `buy_volume`; `sell_volume = volume − buy_volume`). If `candle-ext` is unavailable for the asset (e.g. spot — see §11), buy/sell columns are unavailable and EqI is rejected at eligibility-check time, not during aggregation.
- **From `ticks`:** `O` = first tick price; `H` = max tick price; `L` = min tick price; `C` = last tick price; volumes summed; buy/sell split derived per-tick from `is_buyer_maker` (§5.6). No `candle-ext` join needed.
...
In this codebase, taker-buy data does **not** live on the candle file (`candles/<YYYY>-<MM>_<tf>.csv`'s schema is only `ts, o, h, l, c, vol` — see §5.5). It lives in a parallel side feed named `candle-ext` (`HistoryLoader.Domain/FeedNames.cs:6`), columns `quote_vol, trade_count, taker_buy_vol, taker_buy_quote_vol`, written **only for futures assets** by `BinanceFuturesClient` (`HistoryLoader.Infrastructure/Binance/BinanceFuturesClient.cs:19`). Spot-asset candles ship without `candle-ext` today. The compatibility matrix splits Spot vs. Perp accordingly:

| Source kind | Asset class | Eligible types | Notes |
|-------------|-------------|----------------|-------|
| Tick | any | EqT, EqV, EqD, EqI, (future) Range, Renko | All types; highest fidelity. |
| OHLCV_TimeBar (`candles/`) **+ `candle-ext` side feed** | Perp / Future | EqT, EqV, EqD, EqI* | Perps have `candle-ext` populated by `BinanceFuturesClient`. EqI* persists `imbalance_reconstruction_method = "m1_taker_buy_proxy"` and surfaces the UI fidelity warning (§Q2 resolved). |
| OHLCV_TimeBar (`candles/`) without `candle-ext` | Spot | EqT, EqV, EqD | EqI **not buildable** until tick ingestion lands (Phase 2a). The `/aggregation-options` endpoint must omit EqI for spot assets that have no `candle-ext` directory. |
| OHLCV_TimeBar with OHLC only, no V | any | (future) Range, Renko | Volume-based not buildable. |
| OHLCV_AltBar | any | none | Re-aggregation forbidden in v1. |
| Side (funding, OI, mark, **`candle-ext`**) | any | none | "Aggregate" button disabled in UI. `candle-ext` is consumed *with* the matching candle feed but is not itself a primary source. |

The matrix is exposed via `/aggregation-options` (§7.3); the eligibility check inspects the asset's `feeds.json` for the presence of a `candle-ext` entry covering the requested interval before listing EqI as compatible. The UI renders the Type dropdown options dynamically without hard-coded lists.

> **Spot/EqI follow-up.** When spot tick ingestion (Phase 2a) lands, EqI becomes available for spot via the tick path. Adding spot `candle-ext` (taker-buy on `/api/v3/klines`) is also possible — Binance Spot klines do return `taker_buy_base_volume` — but is out of scope for this TRD; track as a HistoryLoader follow-up if/when the spot proxy variant is needed before tick ingestion ships.
...
1. **Phase 1 — Storage & HistoryLoader core + loader signature change.** Storage convention, accumulators (`EqT`, `EqV`, `EqD`), `feeds.json` extension with fidelity, **per-`feed_id` aggregation mutex with 423 Locked semantics** (§7.4). Time-bar source only (`1m`/`1h`/`1d`). **Includes the `IInt64BarLoader` signature change to `DataFeedDescriptor`-based loading** (§10.5) so Phase 4 isn't a forced refactor of half the Application layer. Phase 1 also covers these explicitly-greenfield items that the original draft folded into "core":
   ...
   - **§Q8 `CsvFeedSeriesLoader` NaN-sentinel update (two-pronged).** The current loader (`CsvFeedSeriesLoader.cs:108-128`) treats two cases differently: missing columns (`:119-123`) zero-fill, and empty/unparseable cells (`:124-128`) skip the entire row. Phase 1 adds an **empty-string → `NaN`** path for sidecar-declared feeds while leaving the missing-column zero-fill and the unparseable-value row-skip behaviour intact for existing time-bar / side feeds. The change is gated on `kind: side` with a `.flow` feed-id, so funding-rate / OI / candle-ext readers are byte-identical to today. Regression tests cover: malformed time-bar row still skipped; sidecar empty cell yields `NaN` row-kept; sidecar `"abc"` cell still skips. See §15-Q8 for full rationale.
...
1. **Existing `candles/*.csv` schema** — **resolved.** Header `ts, o, h, l, c, vol` (all `long`, ts epoch ms), partitioned monthly as `<YYYY>-<MM>_<TimeframeCode>.csv` (underscore between month and timeframe code, **not** dash). Confirmed against `PartitionedCsvBarLoader.GetPartitionPath` at `Infrastructure/History/PartitionedCsvBarLoader.cs:107` (`$"{month:yyyy-MM}_{intervalStr}.csv"`). Aggregated bar files use the **same** schema (§5.5) — alt bars are drop-in `Int64Bar` consumers.
...
8. **Empty cells in sidecar CSVs** — **resolved (with corrected scope).** Empty string in a sidecar CSV cell decodes to `double.NaN`; bar-file cells remain non-nullable `long`. The current `CsvFeedSeriesLoader` (`Infrastructure/History/CsvFeedSeriesLoader.cs:108-128`) handles two distinct cases that the original Q8 wording conflated:

   1. **Missing column** (`valueIdx >= parts.Length`, line 119–123): row has fewer fields than the header → fills with `0d` and increments `missingColumnRows`.
   2. **Present-but-unparseable cell** (line 124–128, includes the empty-string case `","`): hits `parseFailed = true` and the **entire row is skipped**.

   Phase 1's change is therefore **two-pronged**, not a one-line swap:

   - **Case 2 (empty cell):** decode empty string to `double.NaN` and *keep the row* (today the row is discarded, which silently truncates timestamp coverage on sidecar feeds). Other unparseable values (e.g. `"abc"`) still trigger row-skip — this preserves the current malformed-input safety for time-bar feeds. The change is gated on the schema being a sidecar (`kind: side` whose feed-id ends in `.flow`) so existing side feeds without empty cells see no behavioural change; time-bar paths are also unaffected.
   - **Case 1 (missing column):** keep `0d` for backwards compatibility with existing side feeds (funding rate, OI) where a missing column means "exchange returned no value" and zero has historically been treated as the no-value sentinel. *Sidecars never emit short rows* (the writer always emits all declared columns, with empty cells where unavailable — see §5.5), so case 1 is functionally unreachable for sidecar feeds; the `0d` behaviour is retained only to avoid touching the time-bar / side-feed read path.

   Net behaviour: sidecar reads return `NaN` for unavailable cells; existing side-feed reads are byte-identical to today. `FeedSeries.Columns` stays `double[][]` — no schema migration. Strategies that read sidecar columns must guard with `double.IsNaN(v)`; the `IFeedContext.TryGetPrimarySidecar` example in §10.4 shows the pattern. Phase 1 must add regression tests covering: (a) malformed time-bar feed row → still skipped; (b) sidecar row with empty cell at position N → row kept, column N is `NaN`, other columns parse correctly; (c) sidecar row with `"abc"` → row skipped (unchanged from today).

   Rejected alternatives: (a) parallel `bool[][]` mask on `FeedSeries` (heavy; adds a column-major allocation per feed that strategies almost never use); (b) omit columns entirely when source lacks data (forces sidecar schemas to be source-dependent — a feed registered as having `signed_imbalance` should always *have* the column even if some rows are unavailable, so the column index resolved at strategy `OnInit` stays stable across the dataset).
9. **Sidecar feed-key namespacing** — **resolved.** Replaced the string-keyed approach with a typed `IFeedContext.TryGetPrimarySidecar(out double[] values)` accessor + `PrimarySidecarSchema` descriptor (§10.4). The accessor signature deliberately mirrors the existing `TryGetLatest(string, out double[] values)` shape (`Domain/Strategy/IFeedContext.cs:16`) — same shared-buffer ownership rule (do not retain the array across bars), same column-index workflow, same plugin-ABI footprint. No user-facing string key, no namespace reservation, no collision possible. Side feeds named in `feeds.json` continue to be reachable via `TryGetLatest(feedKey, ...)` as today; the new accessor is exclusively for the primary alt bar's auto-bound sidecar.
