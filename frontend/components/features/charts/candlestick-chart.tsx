...
interface CandlestickChartProps {
  // Incremental mode (debug) — per-buffer data
  candles?: CandleData[];
  debugIndicatorBuffers?: Map<string, DebugBufferPoint[]>;
  debugIndicatorMeta?: Map<string, DebugBufferMeta>;
  debugTrades?: DebugTrade[];
  // Bulk mode (report) — overlay-only indicators
...
export const CandlestickChart = forwardRef<CandlestickChartHandle, CandlestickChartProps>(
  function CandlestickChart({
    ...
    useEffect(() => {
      ...
      // Bulk indicators — only overlay (chartId is null/undefined)
      if (bulkIndicators) {
        let colorIdx = 0;
        for (const [name, series] of Object.entries(bulkIndicators)) {
          if (series.chartId != null) {
            colorIdx++;
            continue;
          }
    ...
    // Incremental indicators — only overlay buffers (chartId === null)
    useEffect(() => {
      if (!debugIndicatorBuffers || !chartRef.current) return;

      let colorIdx = 0;
      for (const [key, points] of debugIndicatorBuffers) {
        const meta = debugIndicatorMeta?.get(key);
        // Skip non-overlay buffers
        if (meta && meta.chartId !== null) {
          colorIdx++;
          continue;
        }
        ...
        if (!series) {
          const measure = meta?.measure ?? "price";
          const priceScaleId = measure === "price" ? "right" : key;
    ...
    }, [debugIndicatorBuffers, debugIndicatorMeta]);
