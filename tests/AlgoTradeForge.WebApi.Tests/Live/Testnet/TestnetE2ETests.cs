...
public sealed class TestnetE2ETests(TestnetApiFactory factory) : IDisposable
{
    ...
    public async Task FullHttpLifecycle_StartStreamOrderStop()
    {
        ...
        var startResponse = await _client.PostAsJsonAsync("/api/live/sessions", request, Json);
        Assert.Equal(HttpStatusCode.Accepted, startResponse.StatusCode);

        var startBody = await startResponse.Content.ReadFromJsonAsync<LiveSessionSubmissionResponse>(Json);
        ...
        await strategy.OrderTestsCompleted.Task.WaitAsync(TimeSpan.FromSeconds(180));
        ...
        // 6. Stop session
        var stopResponse = await _client.DeleteAsync($"/api/live/sessions/{sessionId}");
        Assert.Equal(HttpStatusCode.OK, stopResponse.StatusCode);

        // 7. Verify stopped — session should be removed from store
        var afterStop = await _client.GetAsync($"/api/live/sessions/{sessionId}");
