...
| ↳ *single-session internals (IB; see `docs/superpowers/specs/2026-06-20-livehost-collection-execution-design.md`)* | one process, four planes (ingest/archival/dispatch/execution), bounded channels, tuned GC | **data plane keyed by instrument** (`ITickRouter` + `IStrategyDispatch`, shared `IBarAccumulator`s) decoupled from **execution plane keyed by account** (`IOrderRouter`); data account and order account are independent axes (a no-data account B can run strategies on account A's feed); seams are in-process now, cross-node (Valkey/Garnet Streams) later | — (internal refactor of the LiveHost extraction; M3 + M6) |
...
**Feed placement rule — backfillability:** feeds that can be re-fetched anytime are collected per site by that site's instance — a gap from the workstation being off a week self-heals on the next catch-up run. After the Phase-3 archive audit this covers klines, OI, ratios, taker-volume, and funding-rate (all archive-replenishable). Only feeds with no archive source — `liquidations` and spot `book-ticker` — are collected 24/7 by HistoryLoader@cloud and reach the local backend via pull-sync. Sync include rules therefore cover only these un-backfillable feeds + config — never feeds the local instance can recover itself.

**Cloud profile ↔ `Eager` linkage:** archive-replenishable ≠ available-in-real-time — the public archive lags ~1 day. So for live warm-up the cloud instance still keeps warm-up-critical feeds (klines, and optionally ticks/funding) collecting via the per-feed `"Eager": true` override (Phase-2 mechanism), independent of their replenishable-and-therefore-lazy default. Replenishability governs *recoverability*; `Eager` governs *real-time freshness* — they are orthogonal.
...
**Raw md relay (single-session venues only, per Q7):** LiveHost appends normalized raw market events to LiveHost-owned prefixes (`live-md/{venue}/...` — same prefix-ownership model as its session event logs). The relay is an append-only **framed** log, split by feed class: **binary for tick feeds** (`live-md/{venue}/ticks/...` — 64 B versioned header + fixed-width **40 B frames** `frameType1/aggressor1/reserved6/ts8/price8/qty8/seq8`, one uniform width for Trade/Heartbeat/SessionBoundary; ~4–5× smaller and far lower GC than JSON at firehose rates of ~1000 instruments) and **JSONL for low-rate order/fill/session events** (`live-md/{venue}/events/...` — inspectability matters, volume is trivial). Both carry **periodic heartbeat + session-boundary markers** so HistoryLoader can distinguish "producer was down" from "market was quiet" (without them, gap auditing is guesswork); a small `dump-ticks` CLI restores human-inspection of the binary log. HistoryLoader@cloud tails each as one more collector with a persisted `{cursor}` under CAS (the M6 incremental-aggregation pattern) and writes canonical partitions + manifest entries itself. **LiveHost publishes raw, HistoryLoader remains the sole canonicalizer** — LiveHost never touches partitions or `feeds.json`, so the single-writer model is preserved at prefix granularity. Losslessness is a durability property, not a format one: append → `fsync` on rotation → bounded ingest channel with disk-backed spill, so a stalled S3 push applies backpressure but never drops or OOMs. The file is the queue: archival ingestion tolerates minutes of latency, and a durable append log gives ordering + replay + restart-safety with zero operational surface. Implemented with the first single-session connector, not before; LiveHost-internal architecture detailed in `docs/superpowers/specs/2026-06-20-livehost-collection-execution-design.md`.
...
- **Concurrent venues (Binance, Bybit, …):** LiveHost and HistoryLoader connect to the exchange independently. Connections are practically free (order of 1024 streams per spot WS connection, ~200 futures; REST weight budgets in the thousands per minute vs. light 5m/15m/daily polling), REST polling of closed data is self-healing (a missed cycle is repaired by the next fetch), and the failure domains stay decoupled: a LiveHost redeploy can never gap the archive, a collector redeploy never touches a trading session. Within LiveHost, streams ARE shared — one WS connection multiplexed across sessions per exchange, one user-data stream per account (M3 design detail). The LiveHost-internal model that makes this sharing clean is a **data plane keyed by instrument** (`ITickRouter` + `IStrategyDispatch`) decoupled from an **execution plane keyed by account** (`IOrderRouter`): a strategy declares `{ dataSubscriptions[], executionAccount }` as two independent axes, so several strategy sets can consume one account's market data while routing orders to different accounts. Single-account/single-node is the degenerate case of the same model. Detailed in `docs/superpowers/specs/2026-06-20-livehost-collection-execution-design.md`.
