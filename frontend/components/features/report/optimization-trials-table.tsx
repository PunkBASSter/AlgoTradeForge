...
// T048-T050 - Params column, sortable headers, clickable trial IDs
...
/** Format params object as a compact key=value string for the Params column. */
...
/** Metric accessor for sort — maps sort key to a numeric value extractor. */
...
export function OptimizationTrialsTable({
  ...
  return (
    <div className="space-y-2">
      <div className="overflow-x-auto rounded-md border border-border-default" data-testid="trials-table">
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-border-default bg-bg-panel">
              ...
              <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-text-muted">Version</th>
              <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-text-muted">Run ID</th>
              <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-text-muted">Asset</th>
              <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-text-muted">Exchange</th>
              <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-text-muted">TF</th>
          ...
          <tbody className="divide-y divide-border-subtle">
            {sortedTrials.length === 0 ? (
              <tr>
                <td colSpan={17} className="px-4 py-8 text-center text-text-muted">
                  No trials found
                </td>
              </tr>
            ) : (
              sortedTrials.map((row) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-text-primary">{row.strategyVersion}</td>
                  <td className="px-4 py-3 text-text-primary">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <td className="px-4 py-3 text-text-primary">{row.dataSubscriptions[0]?.assetName}</td>
                  <td className="px-4 py-3 text-text-primary">{row.dataSubscriptions[0]?.exchange}</td>
                  <td className="px-4 py-3 text-text-primary">{row.dataSubscriptions[0]?.timeFrame}</td>
                  <td className="px-4 py-3 text-text-primary">
