...
**Cross-project dependency note:** `AlgoTradeForge.HistoryLoader.Infrastructure` references `AlgoTradeForge.Storage` (for `LocalFileStorage` / `S3FileStorage` / `LocalTailIndex` / `S3TailIndex` + the `FileStorageFactory` DI helpers); the abstractions (`IFileStorage`, `IPartitionTailIndex`, etc.) live in `AlgoTradeForge.Storage.Abstractions` and come transitively via `AlgoTradeForge.HistoryLoader.Application`. HistoryLoader does its own DI registration rather than calling `AddInfrastructure`, which would pull in SQLite repos and live-trading wiring the loader has no use for. The previous arrangement (both hosts reaching into `AlgoTradeForge.Infrastructure` for the IO impls via `[InternalsVisibleTo]`) is gone — the storage cluster is now its own dependency, and the main `Infrastructure` project no longer references AWSSDK or exposes internals to HistoryLoader.
