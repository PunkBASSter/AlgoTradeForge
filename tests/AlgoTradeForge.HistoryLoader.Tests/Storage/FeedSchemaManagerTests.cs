...
public sealed class FeedSchemaManagerTests : IDisposable
{
    ...
    private static FeedCollectionConfig MakeFeedConfig(string name = "funding", string interval = "8h") =>
        new() { Name = name, Interval = interval };
    ...
    public void EnsureSchema_NewFile_CreatesFeedsJson()
    {
        ...
        var config   = MakeFeedConfig("funding", "8h");

        manager.EnsureSchema(assetDir, "funding", config, columns);
    ...
    public void EnsureSchema_ExistingFile_UpdatesFeed()
    {
        ...
        manager.EnsureSchema(assetDir, "funding", MakeFeedConfig("funding", "8h"),
            columns: ["rate"]);

        // Update the same feed with a different column set
        manager.EnsureSchema(assetDir, "funding", MakeFeedConfig("funding", "4h"),
    ...
    public void AtomicWrite_NoPartialFiles()
    {
        ...
        manager.EnsureSchema(assetDir, "funding", MakeFeedConfig(), columns: ["rate"]);
