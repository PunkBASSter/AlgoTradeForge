...
7. **Renko catch-up deferred (path-dependent `ReplayBoundary`)** — the M6 catch-up golden passes for the 7 reset-at-bar families but **Renko diverges**: its cross-bar `_pendingVolume` is not reconstructed by replay-from-the-last-completed-bar (golden shows +3 volume on the straddling brick; OHLC + timestamps match). This is the path-dependent case the extensibility table flagged. **Interim (implemented):** the live resolver **disables catch-up for Renko** alt-bars (they start cold, as before this feature — no warmup/replay/gap-recovery, so no silently-wrong volumes), and the Renko golden case is skipped-with-reason. **Follow-up:** implement Renko's two-tier `ReplayBoundary` — replay from K bricks back to a known-zero-pending point, or seed `_pendingVolume` from the manifest/warmup anchor — then re-enable Renko catch-up and un-skip the golden.
