...
- **State records stay pure data; the codec is a thin shared Domain helper.** The `{typeCode,state}` `System.Text.Json` codec (`AccumulatorStateJson`) lives in **Domain** because it is shared by two non-cross-referencing host stacks — the batch side (`HistoryLoader.Application`) *writes* the partial, the live side (`LiveHost.Application`) *reads* it, and the golden requires byte-identical round-trip; Domain is their only clean common home (the relay's `IFramePayload` serialization sets the precedent for Domain-resident codecs). The records themselves carry **no serialization attributes** (`StateType` supplies the concrete type, so each `state` object deserializes as a plain non-polymorphic record). Only the **`IFileStorage` IO + key-layout store** (`IPartialBarStateStore`) sits at each host's boundary (where §H persistence lives).
